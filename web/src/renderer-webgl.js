...
void main() {
  vec2 px = vec2(gl_FragCoord.x, u_size.y - gl_FragCoord.y);   // top-left origin, like WGSL
  vec2 g = u_center + (px - 0.5 * u_size) * u_texPerPx;
  float h = elevation(g);
...
export class ContourRenderer {
  ...
  constructor(canvas) {
    ...
    this.u = Object.fromEntries(
      ["u_dem", "u_size", "u_center", "u_texPerPx", "u_background", "u_tierCount", "u_tierA", "u_tierB", "u_stopColor", "u_stopElev"]
  ...
  render() {
    ...
    gl.uniform2f(u.u_center, hm.width / 2, hm.height / 2);
    gl.uniform1f(u.u_texPerPx, this.texPerPx());
