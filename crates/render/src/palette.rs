//! Palettes: color schemes from terminals/editors, pywal, or images, and
//! turning a palette into a contour [`Theme`].

use crate::{
    color::{Color, Oklch},
    theme::{LineTier, Paint, Theme},
};
...
fn pick_accent(p: &Palette, accent: &str) -> Result<Color> {
    ...
    let pick = match accent.to_ascii_lowercase().as_str() {
        ...
        other => match other.parse::<usize>() {
            ...
            _ => Some(Color::parse(other).with_context(|| format!("unknown accent '{accent}'"))?),
        },
    };
...
impl Palette {
    pub fn to_theme(&self, opts: &GenerateOptions) -> Result<Theme> {
        let pal_bg = self.background.or(self.ansi[0]).unwrap_or(Color::BLACK);
        ...
        let accent = pick_accent(self, &opts.accent)?;
        ...
        let name = format!(
            "{} ({})",
            self.name.clone().unwrap_or_else(|| "palette".into()),
            match opts.style {
                Style::Subtle => "subtle",
                Style::Vivid => "vivid",
                Style::Mono => "mono",
...
mod tests {
    ...
    fn subtle_theme_is_dimmer_than_vivid() {
        ...
        let subtle = p.to_theme(&GenerateOptions::default()).unwrap();
        let vivid = p
            .to_theme(&GenerateOptions {
                style: Style::Vivid,
                ..Default::default()
            })
            .unwrap();
